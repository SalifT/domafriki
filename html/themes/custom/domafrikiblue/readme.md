# Dom Afriki Blue
Dom Afriki Blue is made to provide as a default style that extends the Social Blue style for DomAfriki Open Social. This is a sub theme of socialbase.

Dom Afriki like Social Blue uses Gulp.js as a task runner, so that it can do many tasks
automatically:
...
We got Dom Afriki Blue theme by duplicating the Social blue theme and renamingit to domafrikiblue as a custom theme for DomAfriki project. We make sure all instances of 'socialblue' are renamed to
'domafrikiblue' to guarantee a proper working theme. socialblue being a contrib theme itself will be updated in the future with new features, so it is best not to make changes that might be lost when updating.
...
libraries with the domafrikiblue variant. This means there is a relation between
...
2. Install the gulp-cli tool globally. Normally, installing a Node.js globally
  ...
  To install gulp's global wrapper, run:
    ...
    ```
  Read the following article to have a better hunderstanding of gulp:
